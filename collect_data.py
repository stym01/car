import socket
import struct
import threading
import csv
import time
import math


# ============================================================
# CONFIGURATION
# ============================================================

HOST = "127.0.0.1"

SAMPLE_RATE_HZ = 5
SAMPLE_INTERVAL = 1.0 / SAMPLE_RATE_HZ

MOTIONSIM_PORT = 4445
OUTGAUGE_PORT = 4444

OUTPUT_FILE = "telemetry.csv"

running = True

motion_latest = None
outgauge_latest = None

# One lock shared by all telemetry threads
data_lock = threading.Lock()


# ============================================================
# HELPERS
# ============================================================

def calculate_speed(vx, vy, vz):
    return math.sqrt(
        vx * vx +
        vy * vy +
        vz * vz
    )


def calculate_direction(vx, vy, vz):

    speed = calculate_speed(vx, vy, vz)

    if speed < 0.1:
        return 0.0, 0.0, 0.0

    return (
        vx / speed,
        vy / speed,
        vz / speed
    )


def safe_float(value, default=float("nan")):
    try:
        return float(value)
    except:
        return default


# ============================================================
# MOTIONSIM LISTENER
# ============================================================

def motion_listener():

    global motion_latest

    sock = socket.socket(
        socket.AF_INET,
        socket.SOCK_DGRAM
    )

    sock.setsockopt(
        socket.SOL_SOCKET,
        socket.SO_REUSEADDR,
        1
    )

    sock.bind(
        (HOST, MOTIONSIM_PORT)
    )

    sock.settimeout(1.0)

    print(
        f"[MotionSim] Listening on {MOTIONSIM_PORT}"
    )

    while running:

        try:
            data, _ = sock.recvfrom(4096)

        except socket.timeout:
            continue

        except Exception:
            continue

        # BNG1 + 21 floats = 88 bytes
        if len(data) != 88:
            continue

        try:
            values = struct.unpack(
                "<4s21f",
                data
            )

        except struct.error:
            continue

        if values[0] != b"BNG1":
            continue

        vx = values[4]
        vy = values[5]
        vz = values[6]

        speed = calculate_speed(
            vx,
            vy,
            vz
        )

        dir_x, dir_y, dir_z = calculate_direction(
            vx,
            vy,
            vz
        )

        sample = {

            "timestamp":
                time.perf_counter(),

            # Position
            "x_m": values[1],
            "y_m": values[2],
            "z_m": values[3],

            # Velocity
            "vx_mps": vx,
            "vy_mps": vy,
            "vz_mps": vz,

            "speed_mps": speed,

            # Linear acceleration
            "ax_mps2": values[7],
            "ay_mps2": values[8],
            "az_mps2": values[9],

            # Direction
            "dir_x": dir_x,
            "dir_y": dir_y,
            "dir_z": dir_z,

            # Orientation
            "roll": values[13],
            "pitch": values[14],
            "yaw": values[15],

            # Angular velocity
            "roll_vel": values[16],
            "pitch_vel": values[17],
            "yaw_vel": values[18],

            # Angular acceleration
            "roll_acc": values[19],
            "pitch_acc": values[20],
            "yaw_acc": values[21],
        }

        with data_lock:
            motion_latest = sample


# ============================================================
# OUTGAUGE
# ============================================================

def outgauge_listener():

    global outgauge_latest

    sock = socket.socket(
        socket.AF_INET,
        socket.SOCK_DGRAM
    )

    sock.setsockopt(
        socket.SOL_SOCKET,
        socket.SO_REUSEADDR,
        1
    )

    sock.bind(
        (HOST, OUTGAUGE_PORT)
    )

    sock.settimeout(1.0)

    print(
        f"[OutGauge] Listening on {OUTGAUGE_PORT}"
    )

    while running:

        try:
            data, _ = sock.recvfrom(4096)

        except socket.timeout:
            continue

        except Exception:
            continue

        if len(data) != 96:
            continue

        # ----------------------------------------------------
        # BeamNG / Live for Speed OutGauge structure
        #
        # 96 bytes total:
        #
        # uint32 time
        # char   car[4]
        # uint16 flags
        # byte   gear
        # byte   plid
        # float  speed
        # float  rpm
        # float  turbo
        # float  engtemp
        # float  fuel
        # float  oilpressure
        # float  oiltemp
        # uint32 dashlights
        # uint32 showlights
        # float  throttle
        # float  brake
        # float  clutch
        # char   display[16]
        # char   id[4]
        #
        # ----------------------------------------------------

        try:

            values = struct.unpack(
                "<I4sHBB7f2I3f16s4s",
                data
            )

        except struct.error as e:

            print(
                "[OutGauge] Packet decode error:",
                e
            )

            continue

        sample = {

            "timestamp":
                time.perf_counter(),

            # ------------------------------------------------
            # Vehicle state
            # ------------------------------------------------

            "gear_index":
                values[3],

            # ------------------------------------------------
            # Engine
            # ------------------------------------------------

            "speed_mps":
                values[5],

            "rpm":
                values[6],

            # ------------------------------------------------
            # Engine temperatures
            # ------------------------------------------------

            "coolant_c":
                values[8],

            "fuel":
                values[9],

            "oil_pressure":
                values[10],

            "oil_c":
                values[11],

            # ------------------------------------------------
            # Driver inputs
            # ------------------------------------------------

            "throttle":
                values[16],

            "brake":
                values[17],

            "clutch":
                values[18],
        }

        with data_lock:
            outgauge_latest = sample


# ============================================================
# CSV FIELDS
# ============================================================

FIELDS = [

    "t_s",
    "seq",

    # Position
    "x_m",
    "y_m",
    "z_m",

    # Velocity
    "vx_mps",
    "vy_mps",
    "vz_mps",

    "speed_mps",

    # Acceleration
    "ax_mps2",
    "ay_mps2",
    "az_mps2",

    # Direction
    "dir_x",
    "dir_y",
    "dir_z",

    # Vehicle
    "mass_kg",

    # Engine
    "rpm",
    "engine_torque_nm",
    "engine_av_rads",
    "engine_load",
    "exhaust_flow",

    # Transmission
    "gear_index",
    "clutch_ratio",

    # Driver input
    "throttle",
    "brake",
    "steering",

    # Wheels
    "wheel_av_fl",
    "wheel_av_fr",
    "wheel_av_rl",
    "wheel_av_rr",

    # Brake temperature
    "brake_temp_fl",
    "brake_temp_fr",
    "brake_temp_rl",
    "brake_temp_rr",

    # Aerodynamics
    "downforce_fl",

    # Temperatures
    "coolant_c",
    "oil_c",

    # Fuel / distance
    "fuel_volume_l",
    "odometer_m",
    "trip_m",
    "altitude_m",

    # Vehicle state
    "ignition_level",
    "parkingbrake",
    "avg_wheel_av",
    "engine_running",
    "damage",

    # Battery
    "battery_voltage_v",
    "battery_current_a",
    "battery_temperature_c",
    "battery_soc",

    # Additional useful values
    "oil_pressure",
]


# ============================================================
# BUILD COMBINED ROW
# ============================================================

def build_row(
    motion,
    gauge,
    start_time,
    sequence
):

    # Start with NaN for every field.
    # This prevents missing values from causing
    # KeyError or malformed CSV rows.

    row = {
        field: float("nan")
        for field in FIELDS
    }

    # --------------------------------------------------------
    # TIME
    # --------------------------------------------------------

    row["t_s"] = (
        time.perf_counter()
        - start_time
    )

    row["seq"] = sequence

    # --------------------------------------------------------
    # MOTION
    # --------------------------------------------------------

    if motion is not None:

        for key in [

            "x_m",
            "y_m",
            "z_m",

            "vx_mps",
            "vy_mps",
            "vz_mps",

            "speed_mps",

            "ax_mps2",
            "ay_mps2",
            "az_mps2",

            "dir_x",
            "dir_y",
            "dir_z"
        ]:

            if key in motion:
                row[key] = motion[key]

        # Orientation
        for key in [

            "roll",
            "pitch",
            "yaw",

            "roll_vel",
            "pitch_vel",
            "yaw_vel",

            "roll_acc",
            "pitch_acc",
            "yaw_acc"
        ]:

            if key in motion:
                row[key] = motion[key]

    # --------------------------------------------------------
    # OUTGAUGE
    # --------------------------------------------------------

    if gauge is not None:

        if "gear_index" in gauge:
            row["gear_index"] = gauge["gear_index"]

        if "rpm" in gauge:
            row["rpm"] = gauge["rpm"]

        if "speed_mps" in gauge:

            # MotionSim is our preferred speed source.
            if math.isnan(
                row["speed_mps"]
            ):
                row["speed_mps"] = (
                    gauge["speed_mps"]
                )

        if "coolant_c" in gauge:
            row["coolant_c"] = (
                gauge["coolant_c"]
            )

        if "oil_c" in gauge:
            row["oil_c"] = (
                gauge["oil_c"]
            )

        if "oil_pressure" in gauge:
            row["oil_pressure"] = (
                gauge["oil_pressure"]
            )

        if "fuel" in gauge:
            row["fuel_volume_l"] = (
                gauge["fuel"]
            )

        if "throttle" in gauge:
            row["throttle"] = (
                gauge["throttle"]
            )

        if "brake" in gauge:
            row["brake"] = (
                gauge["brake"]
            )

        if "clutch" in gauge:
            row["clutch_ratio"] = (
                gauge["clutch"]
            )

    # --------------------------------------------------------
    # ENGINE ANGULAR VELOCITY
    #
    # rpm → rad/s
    # --------------------------------------------------------

    if not math.isnan(
        row["rpm"]
    ):

        row["engine_av_rads"] = (
            row["rpm"]
            * 2.0
            * math.pi
            / 60.0
        )

    # --------------------------------------------------------
    # ENGINE RUNNING
    #
    # Derived conservatively from RPM.
    # --------------------------------------------------------

    if not math.isnan(
        row["rpm"]
    ):

        row["engine_running"] = (
            1
            if row["rpm"] > 100
            else 0
        )

    return row


# ============================================================
# CSV WRITER
# ============================================================

def csv_writer():

    print(
        f"[CSV] Recording at "
        f"{SAMPLE_RATE_HZ} Hz"
    )

    with open(
        OUTPUT_FILE,
        "w",
        newline="",
        encoding="utf-8"
    ) as f:

        writer = csv.DictWriter(
            f,
            fieldnames=FIELDS,
            extrasaction="ignore"
        )

        writer.writeheader()
        f.flush()

        sequence = 0

        start_time = (
            time.perf_counter()
        )

        next_sample = (
            time.monotonic()
        )

        while running:

            now = time.monotonic()

            if now >= next_sample:

                # --------------------------------------------
                # Copy latest data safely
                # --------------------------------------------

                with data_lock:

                    motion = (
                        motion_latest.copy()
                        if motion_latest
                        is not None
                        else None
                    )

                    gauge = (
                        outgauge_latest.copy()
                        if outgauge_latest
                        is not None
                        else None
                    )

                # --------------------------------------------
                # Create combined row
                # --------------------------------------------

                row = build_row(
                    motion,
                    gauge,
                    start_time,
                    sequence
                )

                writer.writerow(row)

                f.flush()

                sequence += 1

                # --------------------------------------------
                # Schedule next 5 Hz sample
                # --------------------------------------------

                next_sample += (
                    SAMPLE_INTERVAL
                )

                # If the computer was paused,
                # don't generate a burst of rows.
                if now > (
                    next_sample
                    + SAMPLE_INTERVAL * 10
                ):

                    next_sample = (
                        now
                        + SAMPLE_INTERVAL
                    )

            else:

                time.sleep(0.002)


# ============================================================
# MAIN
# ============================================================

if __name__ == "__main__":

    print(
        "=============================="
    )

    print(
        " BeamNG Research Data Logger"
    )

    print(
        "=============================="
    )

    print()

    print(
        f"[CSV] Output: {OUTPUT_FILE}"
    )

    print(
        f"[CSV] Sampling: "
        f"{SAMPLE_RATE_HZ} Hz"
    )

    # --------------------------------------------------------
    # Start MotionSim
    # --------------------------------------------------------

    t1 = threading.Thread(
        target=motion_listener,
        daemon=True
    )

    # --------------------------------------------------------
    # Start OutGauge
    # --------------------------------------------------------

    t2 = threading.Thread(
        target=outgauge_listener,
        daemon=True
    )

    t1.start()
    t2.start()

    try:

        csv_writer()

    except KeyboardInterrupt:

        print()
        print(
            "Stopping data collection..."
        )

        running = False

        time.sleep(0.5)

    print()
    print(
        "Dataset saved:",
        OUTPUT_FILE
    )